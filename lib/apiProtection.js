...
export function isDirectBrowserAccess(req) {
  ...
  // 9️⃣ BLOCK: Wildcard accept header without referer
  ...
  if (isProduction) {
    ...
    // Legitimate app requests should have at least referer OR sec-fetch-mode
    if (!referer && !secFetchMode && !requestedWith) {
      return true; // No valid indicators - BLOCK
    }
  }

  // Default: ALLOW in development, require indicators in production
  return false;
}
